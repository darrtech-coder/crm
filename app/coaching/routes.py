...
from .models import CoachingPlan, CoachingAcknowledge, CoachingPlanItem, CoachingPlanTest
...
def list_plans():
    if current_user.role=="AGENT":
        ...
        plans=CoachingPlan.query.filter((CoachingPlan.assigned_to==current_user.id)|
                                        (CoachingPlan.team_id.in_(team_ids))).all()
    else:
        plans=CoachingPlan.query.all()
    return render_template("coaching/list.html", plans=plans)
...
def edit_plan(plan_id):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash("Coaching plan updated.","success")
        return redirect(url_for("coaching.list_plans"))
