...
def edit_question(qid):
    ...
    if request.method == "POST":
        ...
        if "delete_media" in request.form and q.media_path:
            ...
            return redirect(url_for("tests.edit_inline", test_id=q.test_id))
        ...
        if q.type == "mcq":
            ...
            options = request.form.getlist("option")
            # This now correctly receives a list of indices like ['0', '2']
            correct_indices = {int(i) for i in request.form.getlist("is_correct")}
...
def add_questions_bulk(test_id):
    """[FIXED] Save multiple new questions at once."""
    test = Test.query.get_or_404(test_id)
    if current_user.role not in ("MANAGER", "ADMIN", "SUPER_ADMIN"):
        flash("Permission denied", "danger")
        return redirect(url_for("tests.edit", test_id=test_id))
    ...
    i = 0
    while True:
        # Check for the existence of the next question block
        q_key = f"question_{i}"
        if q_key not in request.form:
            break

        qtext = request.form.get(q_key, "").strip()
        qtype = request.form.get(f"type_{i}", "mcq")
        ...
        if qtype == "mcq":
            ...
            correct_indices = {int(val) for val in request.form.getlist(f"is_correct_{i}")}
            
            for idx, opt_text in enumerate(options):
                if opt_text.strip():
                    is_correct = idx in correct_indices
                    opt = TestOption(question_id=q.id, text=opt_text.strip(), is_correct=is_correct)
                    db.session.add(opt)
    ...
    if question_count > 0:
        db.session.commit()
        flash(f"Added {question_count} new question(s).", "success")
    else:
