...
from .models import (
    ...
    BiasLog, LibraryCategory, TrendingItem, LibraryAccess, LibraryProgress # <--- [FIX] Add LibraryProgress here
)
...
import tempfile # Make sure to have this import at the top of your file
...
def upload():
    if request.method == "POST":
        ...
        if not main_file or not allowed_file(main_file.filename):
            flash("Invalid or missing main file.", "danger")
            return redirect(url_for("library.upload"))

        title = request.form.get("title", "").strip()
        if not title:
            base, _ = os.path.splitext(main_file.filename)
        ...
        try:
            ...
            generated_thumb_name = auto_generate_thumbnail(tmp_path, main_file.mimetype, thumb_dir)
            
            custom_thumb_file = request.files.get("thumbnail")
            if custom_thumb_file and allowed_file(custom_thumb_file.filename):
                with tempfile.NamedTemporaryFile(delete=False) as ct_tmp:
                    custom_thumb_file.save(ct_tmp)
                with open(ct_tmp.name, 'rb') as f:
                    thumbnail_path = storage.save(f, subfolder='library/thumbnails', filename=custom_thumb_file.filename)
                os.remove(ct_tmp.name)
            elif generated_thumb_name:
                gen_thumb_full_path = os.path.join(thumb_dir, generated_thumb_name)
                with open(gen_thumb_full_path, 'rb') as f:
                    thumbnail_path = storage.save(f, subfolder='library/thumbnails', filename=generated_thumb_name)
                os.remove(gen_thumb_full_path)

            text_content = extract_text(tmp_path, main_file.mimetype)
            with open(tmp_path, 'rb') as f:
                saved_path = storage.save(f, subfolder='library/main', filename=main_file.filename)

        except Exception as e:
            current_app.logger.error(f"Library upload failed during file processing: {e}")
            flash(f"Error saving file to storage: {e}", "danger")
            return redirect(url_for("library.upload"))
        finally:
        ...
        item = LibraryItem(
            title=title, description=request.form.get("description"), keywords=request.form.get("keywords"),
            filename=saved_path, mime=main_file.mimetype, size=file_size, text_content=text_content,
            creator_id=current_user.id,
            category_id=int(request.form.get("category_id")) if request.form.get("category_id") else None,
            thumbnail=thumbnail_path, unlisted=bool(request.form.get("unlisted")),
            manager_only=bool(request.form.get("manager_only"))
        )
        db.session.add(item)
        db.session.flush()

        for attach_file in request.files.getlist("attachments"):
            if attach_file and allowed_file(attach_file.filename):
                try:
                    attach_file.seek(0, os.SEEK_END); attach_size = attach_file.tell(); attach_file.seek(0)
                    attach_path = storage.save(attach_file, subfolder='library/attachments', filename=attach_file.filename)
                    db.session.add(LibraryAttachment(item_id=item.id, filename=attach_path, mime=attach_file.mimetype, size=attach_size))
                except Exception as e:
                    current_app.logger.error(f"Failed to save attachment {attach_file.filename}: {e}")

        # Access control and prerequisite logic
        ...
        for tid in team_ids:
            if tid: db.session.add(LibraryAccess(item_id=item.id, team_id=int(tid)))
        for uid in user_ids:
    ...
    # [THIS IS THE FIX] Handle the GET request by providing a stub 'item' object.
    stub_item = {"unlisted": False, "manager_only": False}
    ...
    return render_template(
        ...
        item=stub_item  # Pass the stub item to the template
    )
...
def view_item(item_id):
    ...
    # --- [FIX] Correct Prerequisite Enforcement Logic ---
    if current_user.role not in ("ADMIN", "SUPER_ADMIN"):
        prerequisites = LibraryPrerequisite.query.filter_by(item_id=item.id).all()
        unmet_prereqs = []
        if prerequisites:
            # Get a set of all item IDs the user has viewed
            viewed_item_ids = {v.item_id for v in LibraryView.query.filter_by(user_id=current_user.id).all()}
            
            # Check each prerequisite against the set of viewed items
            for pr in prerequisites:
                if pr.prereq_item_id not in viewed_item_ids:
                    unmet_prereqs.append(pr)
        
        if unmet_prereqs:
            # Render the dedicated prerequisite gate page instead of just flashing
            return render_template("library/prerequisites.html", item=item, prerequisites=unmet_prereqs)
    ...
    # Fetch the last saved position for the user
...
def download_item(item_id):
    ...
    # Handle thumbnail requests separately
    if request.args.get("thumb") and item.thumbnail:
        return redirect(storage.get_url(item.thumbnail))

    if not item.filename:
        flash("No file associated with this item.", "danger")
        return redirect(url_for('library.view_item', item_id=item_id))
        
    # Redirect to the public URL provided by the storage manager
    return redirect(storage.get_url(item.filename))
...
def download_attachment(attach_id):
    ...
    if not attach.filename:
        # This case should ideally not happen
        flash("Attachment file not found.", "danger")
        # Redirect back to the parent library item page
        return redirect(url_for('library.view_item', item_id=attach.item_id))
        
    # Redirect to the public URL for the attachment
    return redirect(storage.get_url(attach.filename))
...
def end_session(item_id):
    ...
    # --- [FIX] Use safe_commit to handle potential database locks ---
    if not safe_commit():
        # If the commit fails after retries, log it but don't crash.
        # The session data for this specific event might be lost, which is acceptable.
        current_app.logger.warning(f"Failed to commit library session for user {current_user.id} due to db lock.")
        db.session.rollback() # Explicitly roll back to clean the session state
...
# --- [NEW] AJAX route to search for teams ---
@library_bp.route("/search_teams")
@login_required
@role_required("MANAGER", "ADMIN", "SUPER_ADMIN")
def search_teams():
    q = request.args.get("q", "").strip()
    ...
    # Exclude teams already linked if an item_id is provided
    item_id = request.args.get("item_id", type=int)
    query = Team.query
    if q:
        query = query.filter(Team.name.ilike(like_query))
