...
def compute_score(item_id):
    ...
    avg_rating_raw = db.session.query(func.avg(LibraryRating.overall)).filter(LibraryRating.item_id == item_id).scalar()
    quiz_raw = db.session.query(func.avg(QuizAttempt.score)).filter(QuizAttempt.item_id == item_id).scalar()

    # [FIX] Explicitly convert Decimal types to float before calculation
    avg_rating = float(avg_rating_raw) if isinstance(avg_rating_raw, decimal.Decimal) else (avg_rating_raw or 0.0)
    quiz = float(quiz_raw) if isinstance(quiz_raw, decimal.Decimal) else (quiz_raw or 0.0)

    score = (views * 0.2) + (avg_rating * 2) + (quiz * 1.0)
    return score
